...
function pmTopic(i,eid,act)
{
	...
	var handle_key = eid;
	var p = {
		url:thisSiteURL+"ajax.php?mod=pm&code=view_comment",
		post:{pmid:i,cod:act}
	};
	...
	/*
	 * if('' == eidVal){ var
	 * myAjax=$.post(thisSiteURL+"ajax.php?mod=pm&code=view_comment",{pmid:i,cod:act},function(d){if(''!=d){$("#"+eid).html(d);}}); }
	 * else { $("#"+eid).html(''); }
	 */
}
...
function favoriteTopic(i,act)
{
	...
	var act = 'undefined' == typeof(act) ? '' : act; 
	var eid = 'favorite_'+i;
	var myAjax = $.post(thisSiteURL+"ajax.php?mod=topic&code=favor",
						{tid:i,act:act},
						function(d){
							if(''!=d){
								$("#"+eid).html(d);
...
// }
	...
	//alert(c);
	//alert(thisSiteURL+"ajax.php?mod=topic&code=do_add&act=reply");
    // 开始发布
	var myAjax = $.post(
		thisSiteURL+"ajax.php?mod=topic&code=do_add&act=reply",
		{
			totid:totidv,
			syn_to_sina:ss,
            syn_to_qqwb:qs,
...
{   
    ...
    var myAjax = $.post
    (
		thisSiteURL+"ajax.php?mod=topic",
		{
			code:'mylastpublish',
			ref_mod:thisMod,
			ref_code:thisCode,
		...
		function (d) 
        {
			...
			// document.write(d);
			//var r_s = d.replace(/<span class="ut">(.*?)<\/span\s*>/ig, '<span class="ut" style="color:#888;">刚刚</span>');
			var r_s = d.replaceAll('"index.php', '"'+thisSiteURL+'index.php');
            $('#listTopicArea').prepend(r_s);
...
function deleteTopic(i,eid,view)
{
	...
	options = {
		'onClickYes':function(){
			var myAjax = $.post (
				thisSiteURL+"ajax.php?mod=topic&code=delete",
				{tid:i},
				function (d){
			  		$("#"+eid).remove();
...
function deleteVerify(i,eid,view)
{
	...
	options = {
		'onClickYes':function(){
			var myAjax = $.post (
				thisSiteURL+"ajax.php?mod=topic&code=delverify",
				{tid:i},
				function (d){
			  		$("#"+eid).remove();
			  		$("#ajax_output_area").html(d);
...
function follow(fid,eid,events,follow_button)
{
	...
	$('.'+eid).html('');
	var myAjax=$.post(
    	thisSiteURL+"ajax.php?mod=topic&code=follow",
    	{
    		id:fid,
...
function ReportSub(tid,report_content,report_reason,eid)
{
	...
	var myAjax=$.post(
	thisSiteURL+"ajax.php?mod=topic&code=doreport",
	{
		totid:totid,
...
function modifyTopic(i,eid,types,attach)
{
	...
	var handle_key = eid;
	var ajax_url = thisSiteURL+"ajax.php?mod=topic&code=modifytopic";
	var post = {
		tid:i,
		types:types,
	 ...
	 * thisSiteURL+"ajax.php?mod=topic&code=modifytopic", { tid:i, types:types },
...
function modifyTopicSuccess(tid, type)
{
	...
	var myAjax=$.post(
		thisSiteURL+"ajax.php?mod=topic&code=updatecurrent",
		{
			tid:tid,
			refmod:refmod,
			refcode:refcode
...
function do_modifyTopic(i,imageids,attachids,content,types, handle_key)
{
	...
	var myAjax=$.post(
	thisSiteURL+"ajax.php?mod=topic&code=do_modifytopic",
	{
		tid:i,
		imageid:imageids,
		attachid:attachids,
...
function GroupSubmit(ci,i,act,touid) {
	...
	var myAjax = $.post(
		thisSiteURL+"ajax.php?mod=topic&code=do_group",
		{
			group_name:c,
			gid:i,
			act:act,
...
function del_group(group_id,touid)
{
	...
	var myAjax = $.post(
		thisSiteURL+"ajax.php?mod=topic&code=del_group",
		{
			group_id:group_id
		},
...
function get_group_choose(userid){
	...
	var title = "正在载入...";
	var ajax_url = thisSiteURL+"ajax.php?mod=topic&code=group_menu&to_user="+userid;
	showDialog(handle_key, 'ajax', title, {"url":ajax_url});
/*
 * var eidVal = $("#"+"global_select_"+userid).html();
 * 
 * if(''==eidVal){ var myAjax=$.post( thisSiteURL+"ajax.php?mod=topic&code=group_menu", {
...
function groupState(gid,touid,eid) {
	...
	var myAjax = $.post(
		thisSiteURL+"ajax.php?mod=topic&code=group_fields",
		{
			gid:gid,
			touid:touid
		},
		function (d) {
			...
			var myAjax = $.post(
					thisSiteURL+"ajax.php?mod=topic&code=group_list",
					{
						touid:touid
					},
					function (d) {
...
function get_remark(uid)
{
   ...
   var ajax_url = thisSiteURL+"ajax.php?mod=topic&code=remark&uid="+uid;
   showDialog(handle_key, 'ajax', '设置备注名', {"url":ajax_url}, 300);
   /*
	 * if(''==eidVal){ //设置备注名; var myAjax=$.post(
	 * thisSiteURL+"ajax.php?mod=topic&code=remark", { uid:uid }, function(d) { if(''!=d) {
...
function publishSubmit_remark(ci,buddyid)
{
	...
	var myAjax = $.post
    (
		thisSiteURL+"ajax.php?mod=topic&code=add_remark",
		{
			remark:c,
			buddyid:buddyid
		},
...
function favoriteTag(i,act)
{
	...
	var myAjax=$.post
	(
			thisSiteURL+"ajax.php?mod=topic&code=favor_tag",
			{
				tag:i,
				act:act
			},
...
function Forward(content,forward_tid,tid,check,options)
{
	...
	var myAjax=$.post
    (
    	thisSiteURL+"ajax.php?mod=topic&code=forward",
    	{
...
function get_tag_choose(uid,tc_type,ajax){
	...
	if(undefined==Cache.get(_cache_id))
	{
		...
		var myAjax = $.post(
			thisSiteURL+"ajax.php?mod=topic&code=tag_menu",
			{
				uid:uid,
				type:tc_type
			},
...
function imageUploadifyHTML(tid, content_textarea_id, toeid, iuConfirm)
{
	...
	if(iuConfirm)
	{
		...
		$.post
		(
			thisSiteURL+'ajax.php?mod=uploadify&code=html',
			{
				'tid' : tid,
				'new' : 1,
				'only_js' : 0,
...
function attachUploadifyHTML(tid, topic_textarea_id, toeid, iuConfirm, item, itemid)
{
	...
	if(iuConfirm)
	{
		...
		$.post
		(
			thisSiteURL+'ajax.php?mod=uploadattach&code=html',
			{
				'tid' : tid,
				'new' : 1,
				'only_js' : 0,
...
function downattach(aid)
{
	...
	if (aid > 0)
	{
		var durl = thisSiteURL+'ajax.php?mod=uploadattach&code=down';
		var myAjax = $.post (
			durl,{aid:aid},
			function (d){
				if('' != d) {
...
function set_attach_score(i,id)
{
	...
	{
		var myAjax = $.post(thisSiteURL+'ajax.php?mod=uploadattach&code=score',{id:id,score:score},function(){});
	}
}
...
function topic_view_comment_list(tid,item)
{
    ...
    var myAjax=$.post(thisSiteURL+"ajax.php?mod=topic&code=view_"+item,{tid:tid},function(d){$("#" + eid).html(d);});
}
...
function zfQun(wp ,key)
{
	...
	$.get(
		thisSiteURL+'ajax.php?mod=qun&code=widgets&op=my_qun&type=1&key='+key+'&random='+Math.random(),
		{},
		function (d) {
			$("#"+wp).html(d);
...
{// alert(uid);return false;
	...
	var _cache_id = uid;
		...
		var myAjax = $.post(
			thisSiteURL+"ajax.php?mod=topic&code=usermenu",{
				uid:uid
			},
			function (d) {
				if(''!=d) {
...
function get_at_user_choose(nickname,obj){
...
{   
	...
	if(videoid > 0) {
		$.post(
			thisSiteURL+'ajax.php?mod=topic&code=delete_video',
			{
				id:videoid
			},
			function (d) {				
...
function follower_choose(uid,nickname,types,template)
{
	...
	var ajax_url = thisSiteURL+'ajax.php?mod=topic&code=follower_choose';
	var post = {
					uid:uid,
					nickname:nickname,
					types:types,
	...
	if (types == 'lahei') {
		...
		var myAjax=$.post(
		thisSiteURL+"ajax.php?mod=topic&code=follower_choose",
			{
				uid:uid,
				nickname:nickname,
				types:types,
...
{ 
	...
	var myAjax = $.post(
		thisSiteURL+"ajax.php?mod=topic&code=doblacklist",
		{
			uid:uid,
			touid:touid,
			types:types,
...
function user_tag(tagid,tag_name,types)
{
  ...
  if('add' == types)
  {
	...
	var myAjax = $.post(
		thisSiteURL+"ajax.php?mod=topic&code=user_tag",
		{
			tagid:tagid,
			tag_name:tag_name,
			types:types
...
function del_tag(tag_id)
{
	...
	var myAjax = $.post(
		thisSiteURL+"ajax.php?mod=topic&code=del_tag",
		{
			tag_id:tag_id
		},
		function (d) {
...
// return false;
	...
	var myAjax = $.post(
		thisSiteURL+"ajax.php?mod=reminded&code=show",
		{
			uid:r_uid,
			is_uptime:is_uptime
		},
...
function PmSend(touid,to_user,eid,pmid)
{
	...
	var handle_key = eid;
	var ajax_url = thisSiteURL+"ajax.php?mod=pm&code=send";
...
function PmSubmit(i,eid,to_user,topmid, options)
{
	...
	var myAjax=$.post(thisSiteURL+"ajax.php?mod=pm&code=do_add",
	{
		pmid:i,
		message:message,
		to_user:to_user,
...
function topicFaceHTML(eid, insert, getname)
{
	var __TOPIC_FACE_CONFIG__ = 
	{
		...
		'tusiji_face' : ['醒醒','昏迷','耶','怒吼','扭背','顶','抖胸','拜拜','挥汗','无聊','鲁拉','拍砖','揉脸','生日','摊手','洗洗睡','瘫坐','哼','闪闪放','旋转','不行','郁闷','音乐','抓墙','撞墙','歪头','戳眼','飘过','互拍','扎','暗爽','少林寺','我得意','转头','奶瓶','我踢','摇晃','晕厥','笼子','震荡']
	};
...
function pmListChat(msgfromid)
{
	...
	if(''==eidVal)
	{
		var myAjax=$.post
		(
			thisSiteURL+"ajax.php?mod=pm&code=listchat",
			{
				msgfromid:i
			},
			function(d)
...
{	
	...
	if(type == 'btn_dzwc'){
		...
		$.post(thisSiteURL+"ajax.php?mod=item&code=sms",{uid:uid,item:item,itemid:itemid}, function(d){
			if(d == 1){
				if(item == 'live'){
		...
		showDialog(handle_key, 'ajax', htitle, {url:thisSiteURL+"ajax.php?mod=item&code=publishbox&type="+type+"&item="+item+"&itemid="+itemid+"&totid="+totid+"&touid="+touid+"&random="+Math.random()}, 590);
	}else{
		// 如果页面存在了微博发布框则不显示弹出框
		if ($('#i_already').length > 0 && $('#wordCheck').length>0) {
			...
			showDialog(handle_key, 'ajax', '随时随处发微博', {url:thisSiteURL+"ajax.php?mod=misc&code=publishbox&type="+type+"&random="+Math.random()}, 590);
		}
	}
}
...
function ReportBox(tid)
{
	...
	showDialog(handle_key, 'ajax', '举报不良信息', {url:thisSiteURL+"ajax.php?mod=misc&code=report&tid="+tid}, 400);
}
...
function getAppActivity(appMod, appCode, appWpId, options)
{
	...
	appUrl = thisSiteURL+'ajax.php?mod='+appMod+'&code='+appCode+arf;
	var retType = 'html';
	if (options.retType) {
...
{	
	...
	showDialog('del_my_blacklist', 'ajax', '移出黑名单', {"url":thisSiteURL+"ajax.php?mod=topic&code=follower_choose&uid="+uid}, 300);	
}
...
{	
	...
	var myAjax=$.post(
    	thisSiteURL+"ajax.php?mod=topic&code=do_delmyblacklist",
    	{
    		touid:touid
    	},
    	function(d){
...
{	
	...
	showDialog('del_my_fans', 'ajax', '移除粉丝', {"url":thisSiteURL+"ajax.php?mod=topic&code=del_myfans&uid="+uid}, 300);	
}
...
function show_longtext_info_dialog(idval, is_modify, content_id, button_id)
{
	...
	var urlval = thisSiteURL+'ajax.php?mod=longtext';
	if(content_id)
	{
...
function view_longtext(idval, tidval, aobj, TPT_id, ptidv)
{
	...
	else
	{
		$.post
		(
			thisSiteURL+'ajax.php?mod=longtext&code=view',
			{
				'id' : idval			
			},
			function(d)
...
function view_topic_content(idval, sidv, TPT_id)
{
	...
	else
	{
		...
		$.post
		(
			thisSiteURL+'ajax.php?mod=view&code=topic_content',
			{
				'id' : idval,
				'sid' : sidval,
				'TPT_' : TPT_id,
...
{	
	...
	var myAjax = $.post
    (
		thisSiteURL+"ajax.php?mod=topic&code=qmd",
		{	
			// 签名档背景图片路径
			qmd_bg_path:c
		},
...
function insert_qmd()
{
	...
	var myAjax = $.post
    (
		thisSiteURL+"ajax.php?mod=topic&code=insert_qmd",
		{	
			// 签名档背景图片路径
...
{	
	...
	var myAjax = $.post
    (
		thisSiteURL+"ajax.php?mod=topic&code=reg_follow_user",
		{
...
{	
	...
	var myAjax = $.post
    (
		thisSiteURL+"ajax.php?mod=topic&code=modify_user_signature",
		{	
			uid:uid,
			signature:signature
...
function modify_user_stely_mod(uid,style_three_tol,get_code,ajax_list,list_uid)
{
	...
	var myAjax = $.post
    (
		thisSiteURL+"ajax.php?mod=user&code=modify_user_three_tol",
		{	uid:uid,
...
{
	window.location.href="http://"+window.location.host+"/do/login.php";
	return;

	var handle_key = 'del_show_login';
	
	showDialog('del_show_login', 'ajax', '快速登录', {"url":thisSiteURL + "ajax.php?mod=topic&code=showlogin"}, 650);	
}
...
{	
	...
	var ajax_url = '';
	if (showCode == 'my_qun') {
		ajax_url = thisSiteURL+"ajax.php?mod=qun&code=widgets&op=my_qun&type=2";
	} 
	else if(ajax_mod == 'validate')
	{
		ajax_url = thisSiteURL+"ajax.php?mod=validate&code="+showCode;
	}
	else {
		ajax_url = thisSiteURL+"ajax.php?mod=user&code="+showCode;
	}
	var post = {
		uid:uid,
		type:ajax_mod
...
function showSeccodeDialog()
{
	...
	showDialog(handle_key, 'ajax', '请补充下面的登录信息', {url:thisSiteURL+"ajax.php?mod=misc&code=seccode&random="+Math.random()}, 300);
}
...
function checkSeccode(seccode, options)
{
	...
	$.post(
		thisSiteURL+'ajax.php?mod=member&code=check_seccode',
		{"check_value":seccode},
		function(r) {
			if ('' != r) {
...
function showFollowGroupAddDialog()
{
	...
	showDialog(handle_key, 'ajax', '创建分组', {url:thisSiteURL+"ajax.php?mod=topic&code=create_group"}, 300);
...
function getAppDescriptionWidgets(type, options)
{
	...
	if (type == 'qun') {
		ajax_url = thisSiteURL+"ajax.php?mod=qun&code=widgets&op=simple_desc";
		if (options.item_id) {
...
function getVoteDetailWidgets(tid, vid)
{
	...
	area.show();
	var myAjax = $.post(
		thisSiteURL+'ajax.php?mod=vote&code=detail',
		{vid:vid,tid:tid},
		function(r) {
			content.html(r);
 ...
 */
function post_class(){
	showDialog('pclass', 'ajax', '发布分类信息', {"url":thisSiteURL+"ajax.php?mod=class&code=classpost"}, 800);	
}
...
function showTopicRecdDialog(options)
{
	...
	var ajax_url = thisSiteURL+"ajax.php?mod=topic&code=recd&tid="+options.tid;
	if (options.tag_id) {
...
function recdTopic(handle_key, form)
{
	...
	$.post(
		thisSiteURL+"ajax.php?mod=topic&code=do_recd", 
		post_data, 
		function(r){
			if (r.done) {
				closeDialog(handle_key);
...
function getMyFollowUser(page, options)
{
	...
	var myAjax = $.post(
			thisSiteURL+"ajax.php?mod=pm&code=pm_follow_user",
			{
				page:page
...
function check_medal_list(uid,member_id,medal_type)
{
	...
	var myAjax=$.post(
    	thisSiteURL+"ajax.php?mod=topic&code=check_medal_list",
    	{
    		uid:uid,
    		member_id:member_id,
...
function vote_ajax_menu(vid,uid)
{
	...
	var template = 'undefined' == typeof(template) ? '' : template;
	var handle_key = "alert_follower_menu_"+uid;
	var ajax_url = thisSiteURL+'ajax.php?mod=validate&code=validate_vote';
	var post = {
					uid:uid,
					vid:vid
...
function music_serach(){
   ...
   if(pages == null){
	   ...
	   $('#music_list').html('<span style="color:red"><img src="./images/loading.gif">正在检索，请稍等...</span>');
		$.post(thisSiteURL+'ajax.php?mod=class&code=xiami',
			{name:music_name.val(),
			page:pages},
			function(d){
				d= eval('('+d+')');
...
function loadtopic(key,order)
{
	...
	if(isLoading && key){
		...
		var myAjax = $.post(thisSiteURL+'ajax.php?mod=topic&code=ajax',{key:key,order:order},function (d){if('' != d){$('#ajaxtopic').append(d);ajaxnum++;}else{isLoading = false;$("#pagehtml").show();}onloading = false;$('#pageinfo').html('');});
	}else{
...
function force_out(uid){
	...
	showDialog(handle_key, 'ajax', '封杀用户', {url:thisSiteURL+'ajax.php?mod=topic_manage&code=force_out&uid='+uid}, 400);
}

function sendemailtoleader(uid,tid,type){
	var handle_key = 'sendemail';
	if(tid < 1 || uid < 1){show_message('请选择要报备的对象');return false;}
	showDialog(handle_key, 'ajax', '报备', {url:thisSiteURL+'ajax.php?mod=topic_manage&code=sendemail&uid='+uid+'&tid='+tid+'&type='+type}, 400);
}

function setFilterRed(){
	document.getElementById('setfiledmsg').style.display = 'block';
	$.post(
		thisSiteURL+'ajax.php?mod=class&code=getfilter&type=verify_list',
		{},
		function(d){
			if(d.done == true){
...
function force_ip(ip){
	...
	$.post(
		thisSiteURL+'ajax.php?mod=topic_manage&code=force_ip',	
		{ip:ip},
		function(d){
